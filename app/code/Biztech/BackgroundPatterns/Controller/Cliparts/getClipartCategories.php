...
namespace Biztech\BackgroundPatterns\Controller\Cliparts;
ini_set('display_startup_errors', 1);

class getClipartCategories extends \Biztech\Productdesigner\Controller\Cliparts\getClipartCategories {
        ...
        try {
            ...
            foreach ($response['clipArtcategories'] as $key => $value) {
                if($value['id'] == $response['default_category_id']){
                    $default_category = $value['id'];
                    break;
                }
            }

            $response['default_category_id'] = $default_category > 0 ? $default_category : $response['clipArtcategories'][0]['id'];

            if($type == 'pattern'){
                if($product->getDefaultBackgroundPatternCategory()!=""){
                    $response['default_category_id']=$product->getDefaultBackgroundPatternCategory();
                }else{
                    $configClipartCat = $this->_pdHelper->getConfig('productdesigner/background_pattern/default_background_pattern_category', $storeid);
                    $response['default_category_id'] = $configClipartCat;
                }
